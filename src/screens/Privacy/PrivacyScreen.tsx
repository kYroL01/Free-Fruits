...
const PRECISION_LABEL: Record<Precision, string> = {
  exact: 'Exact pin',
  blur: 'Blurred to 100 m',
  private: 'Only me (no points until published)',
};
...
export function PrivacyScreen() {
  ...
  return (
    ...
    >
      ...
      <AppText variant="screenTitle">Privacy & location</AppText>
      ...
      <View style={{ gap: 10 }}>
        <Kicker>What we store</Kicker>
        <Card style={{ gap: 10 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            <AppText variant="rowLabel">GPS inside your tree photos</AppText>
            <AppText variant="microLabel" color={tokens.green}>
              KEPT
            </AppText>
          </View>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            <AppText variant="rowLabel">Pins you opened</AppText>
            <AppText variant="microLabel" dim>
              30 DAYS
            </AppText>
          </View>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            <AppText variant="rowLabel">Your live position</AppText>
            <AppText variant="microLabel" color={tokens.green}>
              NEVER STORED
            </AppText>
          </View>
      ...
      <View style={{ gap: 4 }}>
        <Kicker>My trees on the public map</Kicker>
      ...
      <View style={{ gap: 14 }}>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <AppText variant="rowLabel" style={{ flex: 1 }}>
            Keep a trail of my walks
          </AppText>
          <Switch value={trailOptIn} onValueChange={setTrailOptIn} accessibilityLabel="Keep a trail of my walks" />
        </View>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <AppText variant="rowLabel" style={{ flex: 1 }}>
            Share anonymised harvest counts
          </AppText>
          <Switch
            value={harvestOptIn}
            onValueChange={setHarvestOptIn}
            accessibilityLabel="Share anonymised harvest counts"
          />
        </View>
      </View>

      <View style={{ gap: 10 }}>
        <Kicker>Location history · 148 points · 62 days</Kicker>
        <AppText variant="body" dim>
        ...
        <View style={{ flexDirection: 'row', gap: 10 }}>
          ...
          >
            <AppText variant="rowLabel" color={wiped ? tokens.green : tokens.text}>
              {wiped ? 'History wiped' : wipeArmed ? 'Tap again to confirm' : 'Wipe my location history'}
            </AppText>
          </Pressable>
          <Pressable
            accessibilityRole="button"
          ...
          >
            <AppText variant="rowLabel">{exported ? 'SENT' : 'EXPORT'}</AppText>
          </Pressable>
        </View>
      </View>
    </ScrollView>
